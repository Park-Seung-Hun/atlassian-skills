...
| 상태 | 조건 | 분기 |
|------|------|------|
| 매칭 있음 | 파일 로드 성공 + `templates:` 키 존재 + 등록된 템플릿 중 `match` 패턴이 `{SDD_H1}`에 포함되는 항목 1건 이상 | "템플릿 `{이름}`과 매칭되었습니다. 해당 규칙으로 파싱합니다." 안내 후 **1-4로 진행** |
...
```
{indent}[{타입한국어}] #{N} {origin_마커 있으면 공백 구분} {요약_축약} ({priority}{, SP=x}{, 증거=…})
```
...
- **origin 마커**: 기존 규칙 유지 — `auto`는 🤖, `user`는 👤, `doc`은 마커 없음. 요약 텍스트 앞에 공백 구분으로 배치.
