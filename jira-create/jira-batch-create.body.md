...
> **[필수] 본문에 명시되지 않은 사전 호출 금지**
>
> 본 스킬은 아래 Step 1~7과 hub 본문(`jira-create-hub`)에 명시된 호출만 실행한다. 본문에 없는 임의 호출을 자체 추가하는 것을 금지한다. 특히 다음 패턴은 명시 금지:
>
> - **이슈 타입 사전 검증** — `jira_batch_create_issues({validate_only: true})`로 인스턴스의 이슈 타입 이름 9가지(Epic/에픽/Story/스토리/Task/작업/Sub-task/하위 작업/Subtask 등)를 미리 호출하는 것을 금지. 이슈 타입은 Step 2의 매핑(SDD 템플릿) + Step 5의 ISSUE_TYPE_MAP 확보 절차로 결정한다.
> - **assignee 조기 호출** — `jira_search(jql="assignee = currentUser()")`는 **Step 5 ISSUE_TYPE_MAP 확보 직후 1회만** 호출한다(L263 참조). Step 0/1/2/3/4 중에 미리 호출하지 않는다.
> - **본문에 없는 ad-hoc 검증/탐색** — "효율성"/"안전성" 명목으로 본문 외 호출을 추가하지 않는다. 호출이 추가로 필요하다고 판단되면 사용자에게 알리고 본문 보완을 제안한 뒤 본 스킬은 본문 그대로 진행한다.
