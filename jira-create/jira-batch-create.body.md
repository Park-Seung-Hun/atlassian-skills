...
```
{indent}[{타입한국어}] #{N} {origin_마커} {요약_축약} ({priority}{, SP=x}{, 증거=…})
```
...
- **괄호 속성**: 우선순위는 항상 표기. SP는 값이 있을 때만 `, SP=x`. 증거는 값이 있을 때만 `, 증거=…`. 세 항목 모두 `(none)`이면 괄호 전체 생략.
...
각 PBI 호출 3 완료 시마다 출력: `✅ {KEY} ({타입한국어}) 후처리 완료 ({n}/{N})` (n = 현재 인덱스, N = 총 건수)
...
각 Sub-task 호출 3 완료 시마다 출력: `✅ {KEY} (하위 작업) 생성 완료 ({n}/{N})` (n = 현재 인덱스, N = 총 건수)
