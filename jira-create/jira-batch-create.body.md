...
AskUserQuestion: "PBI/Sub-task의 증거 형태 기본값을 선택하세요."
- "{추천 증거 형태} (추천)" / "PR 링크" / "Confluence 문서 링크" / "데모 자료 링크" / "스크린샷·로그" / "개별 지정"

추천 기준:
- SDD에 코드 파일 경로(`→ file_path`)가 포함된 이슈가 과반이면 → "PR 링크" 추천
- SDD에 설계 문서/조사 성격 Phase가 과반이면 → "Confluence 문서 링크" 추천
- SDD에 UI 컴포넌트 파일이 과반이면 → "스크린샷·로그" 추천
- 판단이 어려우면 → "PR 링크" 추천 (기본 fallback)

> Epic의 증거는 PBI와 다른 수준이다. Epic에는 최종 산출물(배포 URL, 릴리즈 노트, 완료 보고서 등)을 증거로 사용한다. PBI 공통 기본값과 별도로, Epic 증거는 Step 3-4에서 개별 수집한다.
...
- **Epic**: 하위 PBI 전체 완료에 더해, 비즈니스 목표 달성 상태를 판정 기준으로 작성한다. 예: "소셜 로그인을 통한 가입 전환율이 측정 가능한 상태가 된다"
- **Story**: User Story의 "독립 테스트" 항목 + 사용자 관점의 행동과 결과를 명시한다. 예: "사용자가 X를 하면 Y가 된다"
- **Task**: 기술적 완료 기준을 명시한다. 예: "파일이 생성된다", "인터페이스가 정의된다"
- **Sub-task**: 인수 기준이 아닌 **완료 조건**으로 작성한다. "구현 완료", "테스트 통과" 같은 기술적 마침표를 기준으로 한다. Sub-task의 완료 조건은 부모 PBI의 인수 기준을 달성하기 위한 실행 단위임을 고려한다.
- **Spike**: 학습 완료 또는 의사결정 완료를 판정 기준으로 작성한다. 예: "X 방식의 기술적 적용 가능성 여부가 결론으로 도출된다"
...
Step 2에서 공통 기본값을 선택했으면 PBI와 Sub-task에 일괄 적용한다.
"개별 지정"이면 테이블로 제시 → 수정만 받는다.

**Epic 증거는 별도 수집한다.** Epic의 증거는 PBI 수준과 다르다. 최종 산출물 수준의 증거를 AskUserQuestion으로 수집한다:
- "배포 URL" / "릴리즈 노트" / "완료 보고서" / "승인된 최종 기획서" / "직접 입력"

> Sub-task의 증거는 필요 시 남기며, 부모 PBI의 증거로 갈음할 수 있다. 공통 기본값을 적용하되, 불필요하면 빈 값도 허용한다.
...
SDD 컨텍스트를 바탕으로 추정치를 제안한다. 다음 가이드라인을 참고한다:
- **PBI** (Story/Task): 2~3일 내 완료 가능한 크기. 초과 시 분할 검토를 권고한다.
- **Sub-task**: 가급적 1일 내 완료 가능한 크기.
- **Epic**: 추정치를 부여하지 않는다.
...
Sub-task는 `jira_batch_create_issues`를 사용하지 않는다(6-2 주석 참조). 각 Sub-task마다 아래 2단계 호출 체인을 **순차 실행**한다.
...
**호출 2 — `jira_update_issue` (커스텀 필드 + priority + timetracking + description)**:
```json
{
  "{FIELD_AC}": "1. 완료 조건 1\n2. 완료 조건 2",
  "{FIELD_EV}": "증거 텍스트",
...
- `description`: Step 2 확정된 목적 + 범위(포함/제외) Markdown. AC·증거는 커스텀 필드에 이미 들어가므로 여기에 포함하지 않는다.
- **Sub-task에는 스토리 포인트(`{FIELD_SP}`)를 부여하지 않는다.**
- `parent`는 호출 1에서 설정되었으므로 이 호출에는 포함하지 않는다.

> **Sub-task 예외 규칙 (Phase B와 다른 점)**
> - Sub-task는 Jira 제약상 `jira_create_issue` 시점에 parent를 반드시 포함한다. 자동화 룰은 이때 한 번 발동하고, 호출 2에서는 parent를 건드리지 않으므로 재진입하지 않는다. 따라서 description을 호출 2의 payload에 함께 넣어도 덮어쓰기 위험이 없어 2단계로 압축할 수 있다.
> - **Phase B(PBI)에는 이 2단계 최적화를 적용할 수 없다.** Phase B는 호출 2에서 parent(Epic)를 처음 설정하므로 같은 payload의 description이 자동화 룰에 의해 덮인다. Phase B는 3단계 체인을 그대로 유지한다.
> - 만약 실환경에서 Phase C의 description이 자동화 룰로 덮이는 현상이 관측되면, description을 호출 2에서 제외하고 별도 호출 3(`jira_update_issue`로 description 단독 설정)을 추가하는 3단계 구조로 롤백한다.
> - 자동화 룰이 커스텀 필드까지 초기화한다는 증거는 현재 없다. 만약 관측되면 호출 2를 재실행하는 방식으로 대응한다.

**병렬성**: 같은 부모 PBI 아래 Sub-task를 병렬 호출하지 않는다. 자동화 룰 재진입 타이밍 리스크를 피하기 위해 전체를 순차로 실행한다.

**부분 실패 처리**: 특정 Sub-task의 호출 1/2 중 어느 단계라도 실패하면 해당 Sub-task만 실패/부분 완료로 집계하고, 같은 부모의 나머지 Sub-task와 다른 부모의 Sub-task 생성은 중단 없이 계속한다. 실패 항목은 Step 8 리포트의 `⚠️ 부분 완료` 또는 `❌ 생성 실패`에 반영한다.
...
- "예": 실패 건만 추출하여 Step 6의 해당 Phase부터 재실행
  ...
  - 생성 실패 (Phase C): 이미 단건 경로이므로 해당 Sub-task의 2단계 호출 체인을 그대로 다시 실행
...
- Phase A(Epic) / Phase B(PBI)는 3단계 호출 체인(빈 티켓 → 커스텀 필드+parent → description)의 순서를 반드시 준수한다. Phase C(Sub-task)는 create 시 parent가 이미 포함되어 자동화 룰이 호출 2에서 재진입하지 않으므로 2단계 체인(create with parent → 커스텀 필드+description)으로 처리한다.
...
- PBI의 인수 기준(AC)은 사용자 관점의 결과를 명시한다. Sub-task는 인수 기준 대신 완료 조건(기술적 마침표)을 작성한다.
- Epic의 증거는 최종 산출물 수준(배포 URL, 릴리즈 노트 등)이어야 하며, PBI/Sub-task의 "PR 링크" 수준과 구분한다.
- PBI는 2~3일, Sub-task는 1일 내 완료 가능한 크기를 기준으로 추정한다.
