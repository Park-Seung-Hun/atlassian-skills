...
```
{indent}[{타입한국어}] #{N} {origin_마커} {요약_축약} ({priority}{, SP=x}{, 증거={증거값}})
```
...
- **origin 마커 출력 강제** (요약 결정에 필수):
  - 이슈의 **summary 필드 origin**을 기준으로 선택한다 (트리 라인은 이슈 전체를 대표하므로 summary가 기준).
  - `summary.origin == "auto"` → 요약 앞에 **`🤖 `** 접두 (이모지 + 공백 1칸). 이 마커 없이 요약을 출력하면 규약 위반.
  - `summary.origin == "user"` → 요약 앞에 **`👤 `** 접두. (Step 4 (C-2) 수정 반영 후 해당됨)
  - `summary.origin == "doc"` → 마커 없이 요약만 출력.
  - 실전에서는 SDD에서 요약을 그대로 가져온 경우가 대부분이라 `doc`이 흔하다. 다만 **자동 보강된 필드(AC·제외·비즈니스 목표 문장 등)가 이슈 내에 포함된 경우에도 트리 라인의 마커는 `summary` 기준**임을 혼동하지 말 것. (요약 자체가 자동 생성된 드문 케이스에만 🤖가 붙는다.)
...
2. 수정 결과(origin `user` 마커 👤)가 반영된 **현 시점 상태**를 기준으로 렌더링한다.
...
**진행률 메시지 출력 규약 (반드시 준수)**:

이하 5-2 / 5-3 / 5-4에 명시된 `> 🟢 …` 배경 메시지와 `✅ {KEY} (…) 완료 …` 완료 메시지는 **예시가 아니라 출력 지시**다. 실제 MCP 호출 전후에 사용자 화면에 **반드시 한 줄로 렌더링**한다. 생략·축약·최종 요약 테이블로 대체 금지. Codex/Claude 환경에 따라 출력 방식이 달라도 **해당 문구 자체는 반드시 렌더링**해야 규약 준수로 간주한다.

- 배경 메시지 `> 🟢 …`: 각 Phase 혹은 단계 **진입 시점에 1회** 출력
- 완료 메시지 `✅ {KEY} (…) 완료 …`: 각 이슈의 호출 체인이 끝날 때마다 1회 출력 (Phase B/C는 루프이므로 n/N 카운트가 증가함)
- 생략 시 사용자는 실시간 진행 파악이 불가능하고, 실패 구간 식별이 어려워져 회귀·디버깅 비용이 급증한다. 출력 비용보다 생략 비용이 훨씬 크다.
...
호출 3 완료 후 **반드시 출력**: `✅ {KEY} (에픽) 생성 완료 (1/1)` — 이 한 줄 없이 Phase B로 넘어가면 **규약 위반**.

> 타입 이름은 **Step 2-0의 `ISSUE_TYPE_MAP["Epic"]`** 한국어 값을 그대로 쓴다. Phase 이름과 중복되므로 "Epic"을 별도로 접두하지 않는다.
...
> 🟢 PBI 후처리 중... (호출 2/3)  — 루프 진입 시 한 번만 출력
...
각 PBI 호출 3 완료 시마다 **반드시 출력**: `✅ {KEY} ({타입한국어}) 후처리 완료 ({n}/{N})` (n = 현재 인덱스, N = 총 건수) — 이슈마다 매번 1줄. 루프 끝에 한 번 요약하거나 생략하는 것은 **규약 위반**.

> `{타입한국어}`는 해당 PBI의 `ISSUE_TYPE_MAP` 한국어 값(스토리 또는 작업). Phase B는 Story·Task가 혼재하므로 각 이슈의 실제 타입을 병기한다.
...
각 Sub-task 호출 3 완료 시마다 **반드시 출력**: `✅ {KEY} (하위 작업) 생성 완료 ({n}/{N})` (n = 현재 인덱스, N = 총 건수) — 이슈마다 매번 1줄. 생략·요약 병합 금지.

> Phase C는 Sub-task만 다루므로 타입은 `하위 작업`으로 고정. `ISSUE_TYPE_MAP["Sub-task"]`의 실제 한국어 값을 쓴다(인스턴스 설정에 따라 `하위 작업` 또는 `Sub-task`).
...
- 부분 실패 시 즉시 출력: `⚠️ {KEY} (하위 작업) 부분 실패 — 호출 {x}/3 단계`
