...
2. hub 0-0a 서브루틴 실행.
   ...
   - **재지정 완료 → 통과** → config가 갱신된 상태이며, Step 2 산출물(필드 맵)은 그대로 유지한 채 Step 5로 진행. 재지정된 슬롯이 `(none)`이 된 경우 해당 슬롯의 payload 키는 5-0 정화 로직에서 자연스럽게 빠진다.
   - **비활성화** → 해당 슬롯을 세션 내 `(none)`으로 처리하고 Step 5로 진행.
