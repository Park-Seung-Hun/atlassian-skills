...
> - **출력**: `통과` 또는 `중단`. 재지정/비활성화 분기는 서브루틴 내부 루프로 처리한 뒤 `통과`로 귀결한다.
...
AskUserQuestion (단일 선택):
> "어떻게 처리할까요?"
- **재지정** → 해당 슬롯의 0-1 서브섹션(`0-1-AC` 등) 재진입 → 새 값 확정 → 0-0c의 config 갱신 로직으로 `{{CONFIG_PATH}}` 저장 → 다음 실패 슬롯으로 이동. 재지정이 완료되면 `customfield_probe_passed` 플래그는 유지(동일 세션에서 즉시 통과 반환).
  > **[필수] 0-1 서브섹션을 반드시 거친다**: 재지정 선택 직후 `sed` / `perl` / `Write` / `Edit` 등으로 `{{CONFIG_PATH}}`를 **직접 치환하는 것을 금지**한다. 실패 안내 문구에 참고로 노출된 후보값("현재 JST에서 쓰이는 AC 후보는 …" 등)을 **사용자 확인 없이 그대로 채택하는 것도 금지**한다. 0-1-SP / 0-1-AC / 0-1-EV 중 해당 슬롯 서브섹션을 재진입하여 "**매칭 1개인 경우에도 확인 질문 → 예/아니오 응답 수신**" 또는 "**매칭 0개/2개 이상인 경우 후보 선택 UI**"를 반드시 거친 뒤에만 새 값을 확정한다. 확정된 값의 저장은 0-0c의 config 갱신 로직(Read → 라인 단위 교체 → Write) 경로로만 수행한다.
...
> **[금지] 실패 안내 단계에서의 선제 답**: "미존재" / "다른 프로젝트 전용" 안내 문구에 후보값을 미리 찍어두고 바로 치환으로 넘어가는 것을 금지한다. 안내 문구는 **실패 사실만** 전달하고, 대체 후보는 0-1-{슬롯} 재진입 UI에서만 보여준다. 실패 안내와 재지정 UI를 같은 턴에 합쳐 "후보 X로 바꿀까요?" 류의 단일 질문으로 축약하는 것도 금지(사용자가 `0-1-AC` 재수집 질문을 건너뛰는 계기가 되므로).

모든 실패 슬롯 처리가 끝나거나 검증이 바로 통과하면 `customfield_probe_passed = true`를 세팅하고 **호출 측 본문으로 복귀**한다 (Step 1로 점프하지 않는다 — 호출 지점에 따라 Step 5 또는 Step 6으로 진행).
