...
> - 설정 파일은 **프로젝트 범위**(`<CWD>/{{CONFIG_LOCAL_REL}}`) 또는 **글로벌 범위**(`{{CONFIG_PATH_GLOBAL}}`)에 저장될 수 있다. 0-0에서 탐색 순서에 따라 로드한다.

### 커스텀 필드 설정

커스텀 필드 ID는 Step 0에서 config를 로드하거나 `jira_search_fields`로 자동 조회하여 결정한다.
...
아래 순서로 config 파일을 탐색하여 처음 발견한 파일을 로드한다:

1. **프로젝트 config**: `<CWD>/{{CONFIG_LOCAL_REL}}` — Bash 도구로 `pwd`를 실행해 CWD 확인 후 해당 경로를 Read 시도. 성공하면 "📂 프로젝트 config 로드: `<경로>`"를 출력하고 로드 완료.
2. **글로벌 config**: 프로젝트 config가 없으면 `{{CONFIG_PATH_GLOBAL}}`을 Read 시도. 성공하면 "🌐 글로벌 config 로드: `{{CONFIG_PATH_GLOBAL}}`"를 출력하고 로드 완료.
3. **인라인 fallback**: 둘 다 없으면 0-1로 이동.
...
PROJECT_KEY가 `YOUR_`로 시작하면 유효하지 않은 설정으로 간주하고 0-1 인라인 수집 경로로 이동한다.
...
4. 재탐색된 값은 config 파일에 저장하지 않는다 (1회성).
