...
> **신규 SDD 포맷 등록은 `/jira-batch-create` 첫 실행 중 샘플 SDD 기반으로 자동 흡수된다.** 이 스킬은 이미 등록된 템플릿을 **보정·수정·삭제**하거나 SDD 없이 **직접 명시 등록**하고 싶을 때 쓰는 편집 진입점이다. 일반 사용자가 일상적으로 호출하는 스킬이 아니다.
