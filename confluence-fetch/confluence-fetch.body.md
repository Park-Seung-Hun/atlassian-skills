...
1. `mcp__mcp-atlassian__confluence_get_page(page_id, convert_to_markdown=false, include_metadata=true)` 호출.
   - **반환 본문은 raw storage XHTML** (`content.format: "storage"`)이다. `<ac:structured-macro>`, `<ac:task-list>`, `<ac:parameter>`, `<ac:plain-text-body>`, `<ri:attachment>`, `<ri:page>` 같은 Confluence 네임스페이스 태그가 표준 XHTML(`<p>`, `<h1>`, `<img>`, `<table>` 등) 사이에 그대로 섞여 반환된다. 매크로 치환·단순화는 일어나지 않으므로 `ac:name` attribute 값으로 직접 분기한다.
   - 메타에서 `title`, `space.key`, `version`(숫자), `url`, `attachments` 배열 등을 추출해 frontmatter·첨부 다운로드용으로 보관한다.
2. `mcp__mcp-atlassian__confluence_get_attachments(content_id=page_id)` 호출 → 첨부 메타 리스트(attachment_id, filename, mediaType, size). `get_page`의 `metadata.attachments`가 이미 동일 정보를 담고 있다면 호출 생략 가능.
3. storage XHTML을 스캔해 **실제 본문에서 참조된 attachment ID 집합**을 수집한다. 참조 판정 소스:
   - `<ac:image>` 하위의 `<ri:attachment ri:filename="..."/>`
   - `<ac:link>` 하위의 `<ri:attachment ri:filename="..."/>`
   - 실측상 storage가 `<img src=".../download/attachments/<page_id>/<filename>...">` 형태의 직접 img 태그를 쓰는 경우도 있다 (confluence-write가 업로드한 다이어그램 이미지가 이 형태). 이 경우 `filename`을 attachments 메타와 매칭해 `attachment_id`를 얻는다.
   - `<a href="...attachments...">`에 포함된 attachment URL도 동일 방식으로 매칭
4. 참조된 첨부들의 합계 크기와 개수를 계산한다. 이 값이 Step 3의 임계치 판정 기준이다.
5. 권한 오류(403) / 페이지 없음(404)이면 즉시 중단하고 사용자에게 원인을 명확히 보고한다. 부분 산출물은 남기지 않는다.

---

## Step 2 — Storage XHTML → Markdown 역변환 (보수적 v1 규칙표)

storage XHTML을 순회하며 아래 규칙으로 Markdown을 조립한다. 처리 순서는 **내부(가장 깊은 자식) → 외부** 순. HTML 엔티티(`&amp;`, `&lt;` 등)는 표준 디코딩하고, `<![CDATA[...]]>`는 내용만 추출한다(이스케이프 불필요). 매크로의 `ac:macro-id`, `ac:schema-version` attribute는 변환에 영향이 없으므로 무시한다.

| 원천 요소 (storage XHTML) | Markdown 복원 |
|---|---|
| `<h1>`~`<h6>` | `#`~`######` |
| `<p>`, `<strong>`, `<em>`, `<code>`, `<del>`, `<a href>` | 표준 GFM (`<del>`은 `~~...~~`) |
| `<ul>` / `<ol>` / `<li>` | `- ` / `1. ` (중첩은 2칸 들여쓰기) |
| `<blockquote>`, `<hr/>`, `<br/>` | 표준 GFM |
| `<sub>`, `<sup>` | HTML 그대로 유지 (대부분 뷰어가 인라인 HTML로 렌더) |
| `<ac:structured-macro ac:name="info\|note\|tip\|warning">` + `<ac:rich-text-body>` | GFM alert — info/note → `> [!NOTE]`, tip → `> [!TIP]`, warning → `> [!WARNING]`. 내부 rich-text-body를 재귀 변환한 뒤 각 줄 앞에 `> ` prefix |
| `<ac:structured-macro ac:name="expand">` + `<ac:parameter ac:name="title">{T}</ac:parameter>` + `<ac:rich-text-body>{B}</ac:rich-text-body>` | `<details><summary>{T}</summary>\n\n{B 재귀 변환}\n\n</details>` |
| `<ac:structured-macro ac:name="toc">` | `<!-- toc -->` (위치만 보존, 본문에서 리스트를 생성하지는 않음) |
| `<ac:structured-macro ac:name="code">` + `<ac:parameter ac:name="language">{L}</ac:parameter>` + `<ac:plain-text-body><![CDATA[{C}]]></ac:plain-text-body>` | fenced code block: `` ```{L}\n{C}\n``` ``. `title`/`linenumbers` 같은 추가 파라미터가 있으면 fence 바로 뒤에 HTML 주석(`<!-- title=... -->`) |
| `<ac:task-list>` + `<ac:task>` + `<ac:task-status>{complete\|incomplete}</ac:task-status>` + `<ac:task-body>{B}</ac:task-body>` | complete → `- [x] {B}`, incomplete → `- [ ] {B}`. task-body는 인라인 포맷만 재귀 변환 |
| `<ac:image>` + `<ri:attachment ri:filename="X"/>` (참조 대상 첨부) | 다운로드 성공 시 `![X](<./<slug>.assets/att<id>-X>)`, 실패·임계치 초과 시 `![X](<원격 URL>)`. `ac:alt` 속성이 있으면 alt로 사용 |
| `<ac:image>` + `<ri:url ri:value="http..."/>` | `![alt](<URL>)` — 다운로드하지 않음 |
| `<img src=".../download/attachments/<page_id>/<filename>...">` (storage가 직접 img 태그 사용; 실측에서 발견됨) | filename을 attachments 메타와 매칭해 `attachment_id` 획득 → 다운로드 성공 시 `![alt](<./<slug>.assets/att<id>-filename>)`, 매칭·다운로드 실패 시 원격 URL 유지 |
| `<img src="http외부...">` (외부 URL) | `![alt](<URL>)` — 다운로드하지 않음 |
| `<ac:link>` + `<ri:attachment ri:filename="X"/>` | 다운로드 성공 시 `[{ac:link-body}](<./<slug>.assets/att<id>-X>)`, 실패 시 원격 URL 유지 |
| `<ac:link>` + `<ri:page ri:content-title="T" ri:space-key="K"/>` | Confluence URL 생성 (단일 페이지 스코프라 파일 상대경로 매핑 없음). `losses.internal_page_links` 증가 |
| `<a href="...attachments...">` | 첨부 URL에서 filename 추출 → `<ac:link>+<ri:attachment>`와 동일 처리 |
| `<a href="...Confluence 내부 페이지...">` | Confluence URL 그대로 유지. `losses.internal_page_links` 증가 |
| 단순 `<table>` (colspan/rowspan 없음) | GFM 테이블. `<thead>`/`<tbody>` 구분이 없으면 첫 행을 헤더로 승격 |
| **병합셀 있는 `<table>`** (`colspan` 또는 `rowspan` attribute 존재) | **raw HTML 유지** + `losses.merged_tables` 증가 |
| **다단 레이아웃** (`<ac:layout-section>` + `<ac:layout-cell>`) | **raw HTML 유지** + `losses.multi_column_layouts` 증가 |
| 그 외 `<ac:structured-macro ac:name="X">` (v1 미지원) | `[Confluence macro: X]` placeholder + `losses.unsupported_macros[X]` 증가. rich-text-body가 있으면 뒤에 재귀 변환된 텍스트를 덧붙여도 무방(선택) |
| `<ac:emoticon ac:name="smile">` 등 | shortcode `:smile:` |

**v1 미지원(placeholder 처리)**: info/note/tip/warning/expand/toc/code/task-list/emoticon/image/link 외의 모든 `<ac:structured-macro>`. 대표 예: `status`, `jira`, `children-display`, `user profile`, `excerpt`, `excerpt-include`, `include`, info/note/tip/warning이 아닌 일반 `panel`, LaTeX/수식 매크로 등.
...
4. **모든 공백 → `-`** (연속 공백도 `-` 하나로 축약). 이미지 경로에 공백이 포함되면 Markdown 뷰어 호환성이 떨어지므로 하이픈 치환이 안전하다.
5. 선행 `.` 제거 (숨김 파일 방지)
6. 결과가 빈 문자열이거나 하이픈/점뿐이면 `page-<page_id>` 폴백
7. 최대 120자로 자름
8. 한글/영문/숫자/하이픈/언더스코어/점은 보존 (공백은 4번에서 하이픈으로 치환됨)
...
- 본문에서 참조된 각 `attachment_id`에 대해 **`mcp__mcp-atlassian__confluence_download_attachment`를 개별 호출**한다.
  - `mcp__mcp-atlassian__confluence_get_page_images` 같은 일괄 도구는 **사용하지 않는다** — 참조 여부와 무관하게 전체 첨부를 긁어오므로 스킬 정책과 충돌한다.
- MCP 응답은 이 환경에서 **로컬 bin 파일 경로**를 반환한다. 예시 응답:
  ```
  [Resource from mcp-atlassian at attachment:///att2119401941/diagram-1.png] Binary content (application/octet-stream, 18.4KB) saved to /Users/.../tool-results/mcp-mcp-atlassian-blob-XXX.bin
  ```
  응답 텍스트에서 `saved to ` 뒤의 절대 경로를 추출한다.
- 그 경로를 `cp`로 사이드카 폴더에 **`att<attachment_id>-<sanitized_filename>`** 이름으로 복사한다:
  ```bash
  cp "<saved_path>" "<cwd>/<slug>.assets/att<id>-<sanitized>"
  ```
  base64 디코드가 필요 없다 — MCP 서버가 이미 이진 파일로 저장해둔 상태다.
- 응답이 `saved to` 경로 대신 size exceeded 류 오류 메시지(대개 50MB 초과)이면 그 항목만 건너뛰고 손실 집계의 `download_failures` 리스트에 `filename`을 추가한다. 나머지 첨부는 계속 진행.
- 다운로드 성공한 첨부에 대해서만 본문의 `<ac:image>`/`<ac:link>`/`<img>`/`<a>` 참조를 상대 경로(`./<slug>.assets/att<id>-<name>`)로 치환한다. 실패 항목은 원격 URL을 유지한다.
...
- **극단적으로 긴 페이지 (storage XHTML이 모델 컨텍스트를 초과할 정도)**: 부분 변환 결과물을 남기지 말고, 사용자에게 "페이지 크기가 한 번에 처리 가능한 한도를 초과합니다" 취지로 명확히 보고한 뒤 중단한다.
