...
### 왜 storage XHTML을 변환 원천으로 쓰는가

`confluence_get_page(convert_to_markdown=false)`를 호출하면 **raw storage XHTML** (`content.format: "storage"`)이 반환된다. `<ac:structured-macro>`, `<ac:task-list>`, `<ri:attachment>`, `<ac:parameter>`, `<ac:plain-text-body>` 같은 Confluence 네임스페이스 태그가 표준 XHTML(`<p>`, `<h1>`, `<img>`, `<table>` 등) 사이에 그대로 섞여 있다. 매크로는 서버 측에서 치환·단순화되지 않으므로 `ac:name` 값으로 직접 분기해 변환한다.

- storage XHTML은 Confluence가 내부적으로 저장하는 원본 포맷이라 손실이 최소다. 매크로 type, 파라미터, CDATA 내 코드 블록이 모두 보존된 상태로 도착한다.
- 이 포맷을 변환 원천으로 쓰면 lxml/BeautifulSoup 같은 외부 라이브러리 설치 없이 LLM 인라인 치환만으로 Markdown을 조립할 수 있다.
- Bash는 디렉토리 생성(`mkdir`)과 첨부 파일 복사(`cp`)에만 쓴다. MCP가 첨부를 이미 로컬 bin 파일로 저장해 경로를 반환하므로 base64 디코드 단계는 필요 없다.
...
| Step 1 | `confluence_get_page`로 storage XHTML 원문과 메타를, `confluence_get_attachments`로 첨부 메타 리스트를 수집하고, 본문에서 참조된 첨부 ID 집합과 합계 크기/개수를 계산. |
| Step 2 | storage XHTML을 v1 규칙표에 따라 Markdown으로 역변환. `ac:name` 값으로 매크로 종류 분기. 미지원 요소는 placeholder 처리 + 손실 집계. |
...
- `source_format: storage`는 이 파일이 Confluence의 raw storage XHTML(네이티브 저장 포맷)을 원천으로 변환되었음을 명시한다. 향후 sync 스킬이 변환 방식을 식별하는 데 쓸 수 있다.
...
| `confluence_get_page` | 페이지 storage XHTML 원문 및 메타 수집 (`convert_to_markdown=false`) |
| `confluence_get_attachments` | 페이지에 업로드된 첨부 메타 리스트 수집 |
| `confluence_download_attachment` | 참조된 첨부를 개별 다운로드 (MCP가 로컬 bin 파일로 저장하고 경로를 반환 — 그 경로를 `cp`로 사이드카에 복사) |
...
- **다른 mcp-atlassian 버전 대응**: 현재 규칙표는 `tmaxsoft.atlassian.net` 환경에서 실측된 storage XHTML 반환 동작을 기준으로 작성됐다. 다른 버전의 mcp-atlassian이 `processed_html`(BeautifulSoup 전처리 HTML) 또는 base64 첨부를 반환하는 경우에는 본문의 Step 1·Step 3-D 지시를 환경별로 분기하거나 해당 버전에 맞춰 재작성이 필요할 수 있다. v1은 단일 환경 기준.
- **`confluence_get_page` 메타 필드 경로**: `include_metadata=true` 응답에서 `version`, `url`, `attachments` 배열이 어떤 키 이름으로 오는지 실측 기준으로 본문에 반영했지만, 변경될 수 있으므로 응답 구조에 회귀가 생기면 업데이트가 필요하다.
