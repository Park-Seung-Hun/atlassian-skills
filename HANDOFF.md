...
### D. 로컬 stale 브랜치 정리 ✅ 완료 (2026-04-23)

- [x] `feat/project-scoped-config` — 로컬 삭제, **원격 보존**(`ed7d9af`, 아이디어 보관용). A 브랜치의 hub 개편과 충돌 크므로 되살릴 땐 재작성 수준 리베이스 필요
- [x] `fix/jira-batch-create-critical` — 로컬·원격 삭제 (main에 머지 완료 상태였음)
- [x] `refactor/jira-batch-create-auto-first` — 로컬·원격 삭제 (main에 머지 완료 상태였음)
